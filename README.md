...
- One intended CityGML dataset imported into the connected 3DCityDB database.
...
TILESET_DATASET_GEOMETRIC_ERROR=250
...
Returns database metadata and details for the single connected dataset. The
server reads bounds and feature counts directly from `citydb.feature`; using
`refresh=1` rebuilds the in-memory metadata from the database.

### 3D Tiles

```http
GET /api/citydb/3dtiles/tileset.json
GET /api/citydb/3dtiles/tileset.json?queryId=abc123
GET /api/citydb/3dtiles/tile.b3dm?minLon=-73.98&minLat=40.76&maxLon=-73.95&maxLat=40.79
```
...
The tile pipeline exposes batch metadata for `geometryId`, `featureId`,
`objectId`, `className`, `lod`, `property` and `surfaceType`.
...
The response includes a preview table, `totalRowCount` and a `queryId`.
Passing that `queryId` to the 3D Tiles endpoints filters the rendered buildings
until the registry entry expires.
...
4. Loads the connected database dataset as dynamic 3D Tiles.
...
The app treats the connected database as one dataset. Lineage values are
optional and ignored by the current viewer. If multiple GML files are imported
at the same time, the app will render and query them as one combined dataset.
...
For CityJSON, use the `import cityjson` subcommand with the same connection
options.
...
docker exec 3dcitydb psql -U citydb -d citydb -c \
  "select count(*) as features from citydb.feature;"
```
...
```bash
docker exec 3dcitydb psql -U citydb -d citydb -c \
  "select p.val_lod, count(*) from citydb.property p where p.val_geometry_id is not null group by p.val_lod order by p.val_lod;"
```
...
- The app is currently tuned for one NYC LoD2 dataset in the connected database.
...
- If multiple GML imports remain in the database, the app treats them as one
  combined render/query dataset.
