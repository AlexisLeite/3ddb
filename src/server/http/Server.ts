...
import type { SqlQueryService } from "../sql/SqlQueryService.js";
...
export class Server {
  ...
  private configureServer(server: ViteDevServer): void {
    ...
    server.middlewares.use("/api/citydb/query", (req, res) => {
      void this.handleQuery(req, res);
    });
  ...
  private async handleTileset(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (queryId && !this.sqlQueryService.renderFilter(queryId)) {
      sendJson(res, 404, { error: "SQL query filter expired or does not exist" });
      return;
    }
  ...
  private async handleQuery(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      const result = await this.sqlQueryService.execute(body);
